...
void get__seed(lua_State *L)
{
    ...
    for (i = 1; ; i++) {
        ...
        // 计算的hash值，并对work_processes取模，将散列到不同的队列中
        MurmurHash3_x86_32(, strlen(), HASH_SEED, (void *)&id);
        id %= work_processes;

        // 根据id得到list的key
        memset(key, 0, sizeof(key));
        sprintf(key, "_list_%d", id);

        // 插入Redis队列中
        LOG_DEBUG("rpush %s to %s", , key);
        if (redis_list_rpush(key, ) < 0) {
...
static void create_http_client(http__t *, struct in_addr *sin_addr)
{
    http_client_create(, sin_addr, parse_html);
}
        ...
        if (get_free_connection_n() > connection_threshold) {
            if (redis_list_length(key) > 0) {
                ...
                if () {
                    ...
                    if (http_) {
                        dns_query(http_, create_http_client);
                    }
